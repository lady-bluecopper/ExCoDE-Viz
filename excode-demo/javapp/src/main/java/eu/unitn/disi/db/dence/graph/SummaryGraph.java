...
public class SummaryGraph {
    ...
    private HashIntObjMap<WeightedLabeledEdge> edges;
   

    public SummaryGraph(List<DynamicEdge> seeds, int numSnaps, int numEdgesPerSnap) {
    ...
    public SummaryGraph(int numSnaps, List<WeightedLabeledEdge> seeds,  int numEdgesPerSnap) {
        this.numSnaps = numSnaps;
    ...
    private void initializeEdgeMap(List<DynamicEdge> seeds) {
        this.edges = HashIntObjMaps.newMutableMap();
        seeds.stream().forEach(e -> edges.put(e.getEdgeID(), new WeightedLabeledEdge(e, e.getSupport())));
    }
    ...
    public boolean isAVGDenseSubgraph(HashIntSet comp, double minDen) {
        ...
        System.out.println(getWeightSumOf(comp) * 2 / (numSnaps * numNodes));
    ...
    public boolean isAVGDenseSubgraph(HashIntSet comp, int skipped, int numkSnaps, double minDen) {
        ...
        System.out.println((getWeightSumOf(comp) - skipped) * 2 / (numkSnaps * numNodes));
    ...
    public boolean containsDenseSubgraph(HashIntSet comp, double minDen) {
        ...
        temp.stream().forEach(e -> {
            WeightedLabeledEdge edge = edges.get(e);
...
//                }
    ...
    private Pair<Integer, Integer> _getKEdgeSnaps(HashIntSet comp) {
        ...
        int skipped = 0;
        if (numEdgesPerSnap > 1 && existing.size() < numSnaps) {
    ...
    private Pair<Integer, Integer> getKEdgeSnaps(HashIntSet comp) {
        ...
        HashIntSet existing = HashIntSets.newMutableSet();
        existCount.entrySet().stream()
                .filter(e -> e.getValue() >= comp.size() * numEdgesPerSnap / 100.)
                .forEach(e -> existing.add(e.getKey()));
        int skipped = 0;
        if (numEdgesPerSnap > 1 && existing.size() < numSnaps) {
            skipped = existCount.entrySet().stream()
