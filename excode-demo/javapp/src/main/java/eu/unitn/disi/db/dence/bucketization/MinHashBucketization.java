package eu.unitn.disi.db.dence.bucketization;
...
public class MinHashBucketization {
    ...
    public Collection<Pair<Integer, Integer>> getCandidatePairs(List<DynamicEdge> edges) {
        ...
        List<Pair<Integer, Set<String>>> edgeCodes = edges.parallelStream().map(edge -> new Pair<Integer, Set<String>>(edge.getEdgeID(), minHashes(edge)))
    ...
    public Set<String> minHashes(DynamicEdge edge) {
        int[] mins = new int[numHashFuncs * numHashRuns];
        Set<String> hashCodes = new HashSet<String>();
        Arrays.fill(mins, Integer.MAX_VALUE);
        if (edge instanceof BinaryDynamicEdge) {
            ((BinaryDynamicEdge) edge).getEdgeSeries().stream().forEach(t -> IntStream.range(0, mins.length).forEach(h -> mins[h] = Math.min(mins[h], indices[h][t])));
        } else {
            ((WeightedDynamicEdge) edge).getEdgeSeries().keySet().stream().forEach(t -> IntStream.range(0, mins.length).forEach(h -> mins[h] = Math.min(mins[h], indices[h][t])));
